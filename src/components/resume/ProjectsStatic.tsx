  ...
  keyword: string[];
}> = [
    {
      ...
      content: <>
        ...
        <div>온라인 상품 유통 플랫폼 특성상 적게는 수십만개 부터 수백만 개의 상품 데이터와 주문 데이터를 다루기 때문에 비효율적인 로직은 성능 저하의 큰 요인이 될 수 있습니다.</div>
    ...
    },
    {
      ...
      content: <>
        <div>자사 사이트의 상품을 여러 오픈마켓에서 쉽게 관리할 수 있는 데스크탑 애플리케이션입니다.</div>
        <div>자사 서버에 존재하는 상품 DB를 사용자에게 엑셀로 제공을 하면 오픈마켓별 양식에 맞게 가공을 하거나, 각 마켓의 상품 등록 과정을 거쳐야 하는 번거로움이 있어 손쉽게 상품 관리를 하고자 만들어졌습니다.</div>
        <div>Electron을 사용하여 애플리케이션을 개발하였고 UI는 React를 사용했습니다. 기존에 Class component를 functional components로 리팩토링함으로써 reacdt hooks를 사용할 수 있었고, 이로 인해 재사용이 간편하고, 직관적인 로직을 구성할 수 있었습니다. </div>
        <div>오픈마켓(쿠팡, 11번가, 위메프, 스마트스토어 등)의 상품 관련 API를 이용하여 주요 기능들을 개발 후 모듈화하여 종속성이 낮은 구조로 프로그램 내부 인터페이스에 탑재를 했었습니다.</div>
        <div>그 결과 판로가 넓어짐에 따라 사용자 수의 증가와 매출 향상의 결과를 얻을 수 있었습니다.</div>
    ...
    },
    {
      ...
      content: <>
        <div>B2B 플랫폼의 자사 홈페이지입니다.</div>
        <div>해외 상품을 국내 오픈마켓에 등록부터 발주까지 관리 대행해 주는 솔루션을 개발했습니다.</div>
        <div>DB/API/UI 개발과 기획을 주도하여 진행했습니다. UI는 React를 사용하여 개발했으며, Express.js를 이용한 RESTful API 서버를 만들어 데이터를 통신했습니다.</div>
        <div>필요에 따라 Puppeteer를 사용한 웹 오토메이션으로 통계 데이터를 크롤링 하여 수집해 저장했고, 어드민 페이지에 Chart.js를 이용한 시각화된 데이터를 제공했습니다.</div>
        <div>처음부터 직접 설계와 개발을 모두 진행했고, 같이 프로젝트 담당을 한 동료와도 코드 리뷰와 스터디를 주도하여 진행함으로써 협업을 통해 성장하고 원활한 커뮤니케이션을 다시 한번 배우는 기회였습니다. </div>
        <br />
